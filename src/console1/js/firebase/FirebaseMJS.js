...
export default class FirebaseMJS {
    ...
    getMeetingInfo() {
        this.initDB();
        let self = this
        return self._db.collection('MeetingInfo').get()
            .then((docSnapShot) => {
                let rtnMeetingInfo_list = []
                docSnapShot.forEach(element => {
                    let data = element.data()
                    let meetingInfo = new MeetingInfo();
                    meetingInfo.docId = element.id
                    meetingInfo = Object.assign(meetingInfo, data);
                    ...
                    rtnMeetingInfo_list.push(meetingInfo)
                });
                return rtnMeetingInfo_list
            })
    }
    updateMeetingInfo(meetingId, dataObject) {
        this.initDB();
        let self = this
        let docRef = self._db.collection('MeetingInfo').doc(meetingId);
        return docRef.set(dataObject, {
                merge: true
            })
            .then((docSnapShot) => {
                return docSnapShot
    ...
    batchUpdateTaskInfo(arrayTaskDocId_Object) {
        this.initDB();
        let self = this
        // Get a new write batch
        var batch = self._db.batch();
        arrayTaskDocId_Object.forEach((taskInfo) => {
            let docRef = self._db.collection("TaskInfo").doc(taskInfo.docId);
            batch.update(docRef, taskInfo);
        })
        // Commit the batch
    ...
    addMeetingInfo(meetingInfo) {
        this.initDB();
        let self = this
        let docRef = self._db.collection('MeetingInfo').doc();
        let fsMeetingInfo = Object.assign({}, meetingInfo)
        fsMeetingInfo.fstsCreateDateTime_server = this._firebase.firestore.FieldValue.serverTimestamp();

        return docRef.set(fsMeetingInfo, {
                merge: true
            })
            .then((docSnapShot) => {
                return docSnapShot
    ...
    deleteMeetingInfo = (docId) => {
        this.initDB();
        let self = this
        return self._db.collection('MeetingInfo').doc(docId).delete()
            .then(() => {
    ...
    addTaskInfo(taskInfo) {
        this.initDB();
        let self = this
        let blExist_docId = (taskInfo.docId === null) ? false : true;
        let docRef;
        let fsTaskInfo = dataKits.getPlainObject(taskInfo) //Object.assign({}, taskInfo)
        fsTaskInfo.fstsCreateDateTime_server = this._firebase.firestore.FieldValue.serverTimestamp();

        if (blExist_docId) {
            return self._db.collection('TaskInfo').doc(taskInfo.docId).set(fsTaskInfo, {
                    merge: true
                })
                .then(() => {
                    return fsTaskInfo //docSnapShot
                    //console.log('docSnapShot-->', docSnapShot)
                })
        } else {
            return docRef = self._db.collection('TaskInfo').add(fsTaskInfo)
                .then((docRef) => {
                    //console.log("Document written with ID: ", docRef.id);
                    fsTaskInfo.docId = docRef.id
                    return fsTaskInfo
                })
        }
    }
    getArrayTaskInfo = (meetingId) => {
        this.initDB();
        let self = this
        return self._db.collection("TaskInfo").where('meetingId', '==', meetingId).get()
            .then((snapshot) => {
                let rtnArrayTaskInfo = snapshot.docs.map((item) => {
                    let data = item.data()
                    let newTaskInfo = new TaskInfo();
                    newTaskInfo = Object.assign(newTaskInfo, data)
                    newTaskInfo.docId = item.id;
                    return newTaskInfo //OrderInfo.getOrderInfo_FromDbFormat(data)
                })
                return rtnArrayTaskInfo
            })
    }
    deleteTaskInfo = (docId) => {
        this.initDB();
        let self = this
        return self._db.collection('TaskInfo').doc(docId).delete()
            .then(() => {
                return true //docSnapShot
                //console.log('docSnapShot-->', docSnapShot)
            })
